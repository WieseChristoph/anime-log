...
const Navbar = ({ urlShareId }: NavbarPropsType) => {
    ...
    const getHref = (href: string) => {
        ...
        return `${href}/${urlShareId}`;
    };

    return (
        <header className="sticky top-0 z-40 border-white/10 border-b bg-(--nav) text-(--nav-text) shadow-slate-950/10 shadow-xl">
            <div className="page-frame flex min-h-14 items-center gap-2 py-2">
                ...
                >
                    <Image
                        ...
                        priority
                    />
                    <span className="display-font font-bold text-base tracking-tight sm:text-lg">Anime Log</span>
                </Link>
                ...
                >
                        ...
                        links.map((link) => {
                            ...
                            return (
                                <Link
                                    ...
                                    href={href}
                                    className={cn(
                                        'rounded-xl px-3 py-2 font-semibold text-sm transition',
                                        active
                    ...
                    <Link
                        ...
                        target="_blank"
                        className="rounded-xl px-3 py-2 font-semibold text-slate-300 text-sm hover:bg-white/8 hover:text-white"
                    >
                        About
                    </Link>
                </nav>
            ...
            {mobileOpen && (
                <div className="border-white/10 border-t bg-(--nav) lg:hidden">
                    <nav
                        className="page-frame flex flex-col gap-1 py-3"
                        ...
                        {(status === 'authenticated' || urlShareId) &&
                            links.map((link) => (
                                <Link
                                    ...
                                    onClick={() => setMobileOpen(false)}
                                    className="rounded-xl px-3 py-3 font-semibold text-slate-200 text-sm hover:bg-white/10"
                                >
                                    {link.label}
                                </Link>
                            ))}
