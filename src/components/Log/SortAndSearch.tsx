...
const AscendingIcon = ({ ascending }: { ascending: boolean }) => {
	return ascending ? (
...
const SortAndSearch = ({
	...
	const sortButtonStyle = (order: string) =>
	...
	const onOrderButtonClick = (order: string) => {
	...
	return (
		<div className="font-medium text-center text-gray-500 border-b border-gray-200 dark:text-gray-400 dark:border-gray-700">
			...
			<ul className="flex flex-wrap">
				...
				<li className="mr-2">
					<button
						onClick={() => onOrderButtonClick(orderBy.updatedAt)}
						className={sortButtonStyle(orderBy.updatedAt)}
					>
						Last Update
						{currentOrder === orderBy.updatedAt && (
							<AscendingIcon ascending={ascending} />
						)}
					</button>
				</li>
