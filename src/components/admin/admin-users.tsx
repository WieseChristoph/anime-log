...
const AdminUsers = () => {
    ...
    const getAnimeMangaCount = trpc.anime.getCountByUser.useQuery();
    ...
    // Error Alert
    if (getAllUsers.isError || deleteUser.isError || getAnimeMangaCount.isError || getLastUpdated.isError)
        return (
            <div className="p-5">
                <ErrorAlert
                    message={
    ...
    return (
        <div className="my-4 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3">
            {getAllUsers.data?.map((user) => (
                <Disclosure key={user.id} as="div" className="grow">
                    <DisclosureButton className="ui-not-open:rounded-b ui-not-open:border-b flex w-full items-center gap-2 rounded-t border-x border-t border-gray-300 bg-gray-200 p-4 py-2 shadow-md dark:border-slate-700 dark:bg-slate-800">
                        <ImageWithFallback
                            className="rounded-full"
                            src={user.image || 'https://cdn.discordapp.com/embed/avatars/1.png'}
                    ...
                    >
                        <Disclosure.Panel className="flex flex-col rounded-b border-x border-b border-gray-300 bg-gray-200 pb-4 text-black dark:border-slate-700 dark:bg-slate-800 dark:text-white">
                            <table>
                                <tbody>
                                    ...
                                    <tr className="border-b border-gray-300 dark:border-slate-400">
                                        ...
                                        <td className="text-center">{user.sessions.length || '0'}</td>
                                    </tr>
                                    <tr className="border-b border-gray-300 dark:border-slate-400">
                                        <td className="px-2 py-2">
                                            <b>Total anime</b>
                                        ...
                                        <td className="text-center">
                                            {getAnimeMangaCount.data?.find((e) => e.userId === user.id)?._count._all ||
                                                '0'}
                                        </td>
                                    </tr>
                                    <tr className="border-b border-gray-300 dark:border-slate-400">
                                        ...
                                        <td className="text-center">
                                            {(() => {
                                                const updatedAt = getLastUpdated.data?.find(
                                                    (e) => e.userId === user.id,
                                                )?.updatedAt;
                                            ...
                                            {(() => {
                                                const lastOnline = user.sessions
                                                    .sort((sessionA, sessionB) =>
                                                        dayjs(sessionB.expires).diff(sessionA.expires),
                                                    )
                                                    .at(0)?.expires;
                                                return lastOnline
                                                    ? dayjs(lastOnline)
                                        ...
                                        <td className="flex flex-wrap justify-center gap-x-1 gap-y-0.5 py-2">
                                            {user.savedUsers.map((savedUser) => (
                                                <span
                                                    ...
                                                    className="rounded bg-white px-2.5 py-0.5 text-xs font-medium text-gray-800 dark:bg-slate-900 dark:text-gray-300"
                                                >
                                                    {getAllUsers.data.find((u) => u.id === savedUser.savedUserId)?.name}
                                                </span>
                                            ))}
                                        </td>
                                    </tr>
                            ...
                            <DeleteButton
                                className="mx-auto mt-4 w-2/3 rounded-lg bg-red-700 px-5 py-2.5 text-center text-sm font-medium text-white hover:bg-red-800 focus:ring-4 focus:ring-red-300 focus:outline-none dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900"
