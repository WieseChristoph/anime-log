...
export default function EntryEditorDrawer({ open, initialAnime, onClose, onSave }: EntryEditorDrawerProps) {
            ...
            void searchTitles(query, anime.isManga, controller.signal)
                .then((results) => {
                    ...
                    if (bestMatch?.imageUrl) {
                        setAnime((current) => current.hasCustomImage || current.title.trim() !== query.trim() ? current : { ...current, imageUrl: bestMatch.imageUrl });
                    }
                })
    ...
    const applySuggestion = (suggestion: AnimeSearchResult, useTitle: boolean) => {
        setAnime((current) => ({ ...current, ...(useTitle ? { title: suggestion.title } : {}), imageUrl: suggestion.imageUrl }));
        setQuery('');
        setSuggestions([]);
    };
            ...
            <div className="fixed inset-0 overflow-hidden"><div className="absolute inset-0 overflow-hidden"><div className="pointer-events-none fixed inset-y-0 right-0 flex max-w-full pl-4 sm:pl-10"><Transition.Child as={Fragment} enter="transform transition duration-300 ease-out" enterFrom="translate-x-full" enterTo="translate-x-0" leave="transform transition duration-200 ease-in" leaveFrom="translate-x-0" leaveTo="translate-x-full"><Dialog.Panel className="pointer-events-auto flex h-full w-screen max-w-2xl flex-col bg-(--surface) shadow-2xl">
                ...
                <form className="flex min-h-0 flex-1 flex-col" onSubmit={(event) => void submit(event)}>
                    <div className="flex-1 space-y-7 overflow-y-auto px-5 py-6 sm:px-8">
                        ...
                        <section className="space-y-4"><div><p className="muted text-xs font-bold uppercase tracking-widest">Identity</p><h2 className="display-font mt-1 text-lg font-bold">What are you logging?</h2></div><div className="flex rounded-xl border border-(--border) p-1"><button type="button" onClick={() => update('isManga', false)} className={`flex-1 rounded-lg px-3 py-2.5 text-sm font-bold ${!anime.isManga ? 'bg-(--accent) text-white' : 'muted'}`}>Anime</button><button type="button" onClick={() => update('isManga', true)} className={`flex-1 rounded-lg px-3 py-2.5 text-sm font-bold ${anime.isManga ? 'bg-(--accent) text-white' : 'muted'}`}>Manga</button></div><div className="relative"><label htmlFor="entry-title" className="mb-2 block text-sm font-bold">Title</label><input id="entry-title" className="field" value={anime.title} onChange={(event) => { update('title', event.target.value); setQuery(event.target.value); }} placeholder="e.g. Frieren: Beyond Journey's End" maxLength={250} required autoFocus />{query && !anime.hasCustomImage && <div className="absolute top-full right-0 left-0 z-10 mt-2 overflow-hidden rounded-xl border border-(--border) bg-(--surface) shadow-xl">{searching && <p className="muted flex items-center gap-2 p-3 text-sm"><LoaderCircle size={15} className="animate-spin" /> Searching Kitsu...</p>}{!searching && suggestions.map((suggestion) => <div key={suggestion.id} className="flex min-w-0 items-center gap-3 border-b border-(--border) p-3 last:border-b-0"><img src={suggestion.imageUrl || '/placeholder.jpg'} alt="" className="h-12 w-9 shrink-0 rounded object-cover" /><span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold">{suggestion.title}</span><span className="muted text-xs">Kitsu suggestion</span></span><div className="flex shrink-0 gap-1"><button type="button" className="btn-primary h-6 min-h-6 whitespace-nowrap px-1.5 py-0 text-[0.6rem] leading-none" onClick={() => applySuggestion(suggestion, true)} aria-label={`Use title and cover from ${suggestion.title}`} title="Use title + cover">Title + cover</button><button type="button" className="btn-secondary h-6 min-h-6 whitespace-nowrap px-1.5 py-0 text-[0.6rem] leading-none" onClick={() => applySuggestion(suggestion, false)} aria-label={`Use cover only from ${suggestion.title}`} title="Use cover only">Cover only</button></div></div>)}{!searching && !suggestions.length && <p className="muted p-3 text-xs">No matches. You can keep your custom title and add a cover URL below.</p>}</div>}</div><p className="muted text-xs">Choose <strong>Title + cover</strong> to use both, or <strong>Cover only</strong> to keep your title.</p></section>
