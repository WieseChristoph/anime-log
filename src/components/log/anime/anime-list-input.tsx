...
type AnimeListInputPropsType = {
    initialArray: number[];
    onArrayChange: (changedArray: number[]) => void;
};

const AnimeListInput = ({ initialArray, onArrayChange }: AnimeListInputPropsType) => {
    ...
    useEffect(() => onArrayChangeRef.current(array), [array]);
        ...
        <>
            <div className="flex flex-row">
                <input
                    ...
                    min="1"
                    value={arrayElement}
                    onChange={(e) => setArrayElement(parseInt(e.target.value, 10))}
                />
