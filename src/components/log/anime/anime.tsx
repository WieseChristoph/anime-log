...
function arrayToCompactString(array: number[]): string {
    ...
    for (let i = 1; i < array.length; i++) {
        ...
        if (prev + 1 === curr) {
            ...
            // if not consecutive
            if (prev !== curr) compactString += `-${prev}`;
            compactString += `, ${curr}`;
        }
    }
...
const Anime = ({ anime, onDeleteClick, onEditClick, isSharedLog = false, index }: AnimePropsType) => {
    return (
        <motion.div
            className="w-full"
        ...
        >
            ...
            >
                ...
                {/* Rating */}
                <div className="absolute top-1 right-1 rounded-md bg-linear-to-r from-pink-500 to-orange-400 px-2.5 py-0.5 text-sm font-bold text-white">
                    {anime.rating} / 10
                </div>
                <div className="absolute right-0 bottom-0 left-0 flex h-3/4 flex-col items-center justify-end bg-linear-to-b from-transparent to-black/75 p-3">
                    {/* Title */}
                    <div
                        className={`text-center font-bold wrap-break-word text-white ${anime.title.length > 50 ? 'text-md' : 'text-xl'}`}
                    ...
                    <div className="flex w-full flex-col gap-1 text-sm text-white">
                        <div className="flex flex-row font-semibold">
                            <span>Season&nbsp;</span>
                            <span className="max-h-10 overflow-auto wrap-break-word">
                        ...
                        <div className="flex flex-row font-semibold">
                            <span>Movie&nbsp;</span>
                            <span className="wrap-break-word">{arrayToCompactString(anime.movies) || '-'}</span>
                        </div>
                        <hr />
                        <div className="flex flex-row font-semibold">
                            <span>OVA&nbsp;</span>
                            <span className="wrap-break-word">{arrayToCompactString(anime.ovas) || '-'}</span>
                        </div>
                    </div>
                </div>
            </motion.div>
