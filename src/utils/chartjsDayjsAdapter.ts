...
_adapters._date.override({
    //_id: 'dayjs', //DEBUG,
    formats: () => FORMATS,
    parse: function (value: string | dayjs.Dayjs, format?: TimeUnit) {
    ...
    format: function (time: number, format: TimeUnit): string {
        return dayjs(time).format(format);
    },
    add: function (time: number, amount: number, unit: QUnitType & TimeUnit) {
        return dayjs(time).add(amount, unit).valueOf();
    },
    diff: function (max: number, min: number, unit: TimeUnit) {
        return dayjs(max).diff(dayjs(min), unit);
    },
    startOf: function (time: number, unit: (TimeUnit & QUnitType) | 'isoWeek', weekday?: number) {
        if (unit === 'isoWeek') {
    ...
    endOf: function (time: number, unit: TimeUnit & QUnitType) {
